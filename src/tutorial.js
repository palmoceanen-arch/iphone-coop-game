...
function setupText(slot) {
  ...
  const controls = [
    ...
    ['Рывок', `${key(slot, 'dash')} — быстро отпрыгни от врага или спасай друга.`],
    ['Действие', `${key(slot, 'interact')} — открыть сундук, дверь, алтарь, грядку или костёр.`],
    ['Стройка', `${key(slot, 'buildMenu')} — открой круг построек. ${key(slot, 'attack')} ставит, ${key(slot, 'dash')} отменяет, ${key(slot, 'interact')} поворачивает.`],
  ...
  return `
    ${renderCard('Кнопки героя', `
      <p>Игра сама показывает кнопки для клавиатуры или геймпада. Если взял геймпад — подсказки поменяются.</p>
...
function combatText(slot) {
  return `
    ${renderCard('Бой без страха', `
      <ul>
        <li>У героя есть сердечки здоровья. Если враг ударит тебя, здоровье станет меньше.</li>
        ...
        <li>Ночью врагов больше. Если страшно — строй стены, держись ближе к костру и собирай еду.</li>
        <li>В одиночной игре, если герой упал, нажми «Заново» и начни новую попытку.</li>
      </ul>
    `)}
    ${renderCard('Добыча после боя', `
      <p>Из врагов падает золото и иногда еда. Золото трать в магазине улучшений через ${key(slot, 'shop')}.</p>
    `)}
  `;
}
